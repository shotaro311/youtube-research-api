...
- コメント分析の保存は `台本DB` シートへ `comment_analysis` 行として保存し、同じ `scriptId` を開いた際は保存済みの分析結果を初期表示に使う
...
- `GOOGLE_SHEETS_SPREADSHEET_ID` / `GOOGLE_SHEETS_SHEET_NAME` / `GOOGLE_SHEETS_SCRIPT_DB_SHEET_NAME` / `GOOGLE_SHEETS_COMMENT_DB_SHEET_NAME` / `GOOGLE_APPLICATION_CREDENTIALS` / `GOOGLE_APPLICATION_CREDENTIALS_JSON` は、前後の空白や改行が入っていても吸収して使う
...
- `コメントDB` は `comment_id / script_id / video_id / video_url / title / channel_name / published_at / comment_index / author / comment_text / likes / created_at` を保持し、コメント分析用の正規化データとして使う
