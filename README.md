...
- シート反映時は分析用に `コメントDB` も使い、各コメントを `1コメント = 1行` で保存します。`コメントDB` が無い場合は初回反映時に自動作成します
...
- コメント分析の総評や視聴者分析は `台本DB` の `comment_analysis` 行として保存し、各コメントの感情タグ・視聴者像・心理・個別フィードバックは `コメントDB` の各行にも同期して保存します
