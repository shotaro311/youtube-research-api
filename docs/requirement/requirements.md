...
- `/scripts/[scriptId]?tab=transcript` では `Geminiで整形` ボタンを押すと、元の台本を保存し直さずに、内容を変えないまま見出し追加と話者切り替わり付近の改行調整をした整形版を画面上に表示できるようにする
...
- `Geminiで整形` は `GEMINI_API_KEY` が設定されているときだけ利用できる
- `Geminiで整形` の既定モデルは `gemini-3.1-flash-lite-preview` とする
