...
- `POST /api/v1/videos/analyze`
  - 入力: `rawData`
  - 出力: `transcriptAnalysis?`, `commentAnalysis?`, `transcriptAnalysisError?`, `commentAnalysisError?`
- `GET /api/health`
  - 出力: `status`
...
- 抽出結果カードでは `分析` ボタンから、その場で `台本分析 / コメント分析` を実行し、カード下部でタブ切り替え表示できます
- 抽出結果は各カードごとの JSON 保存に加えて、成功分まとめての ZIP 一括ダウンロードにも対応し、取得できるサムネ画像は ZIP に同梱します
