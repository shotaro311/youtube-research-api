...
- 動画抽出結果では、各カードごとにその動画1件分の JSON を保存できるようにする
- 動画抽出結果の成功分は、動画ごとの JSON ファイルをまとめた ZIP として一括ダウンロードできるようにする
- 一括ダウンロードの ZIP には、取得できる場合は各動画のサムネ画像ファイルも同梱する
- 動画抽出結果カードでは `分析` ボタンから、同じカード内で `台本分析 / コメント分析` をタブ切り替えで表示できるようにする
- 抽出カード内の `台本分析` は、動画の流れ・視聴者心理の変化・寄り添い発言・企画意図・良い部分・改善点・総合評価を表示する
- 抽出カード内の `コメント分析` は、総評・感情比率・視聴者像・視聴者心理・個別コメント分析を表示する
...
### 5.3 `POST /api/v1/videos/analyze`

- 入力
  - `rawData`
- 出力
  - `transcriptAnalysis?: { title: string; flowStages: { stage: string; summary: string; viewerPsychology: string }[]; empathyMoments: string[]; creatorIntent: string; viewerStrengths: string[]; viewerImprovements: string[]; overallScore: number; overallVerdict: string; overallEvaluation: string }`
  - `commentAnalysis?: { title: string; overview: string; positivePercent: number; neutralPercent: number; negativePercent: number; audienceSummary: string; psychologySummary: string; positiveThemes: string[]; negativeThemes: string[]; items: { commentIndex: number; sentiment: "positive" | "neutral" | "negative"; viewerType: string; psychology: string; note: string }[] }`
...
### 5.5 `POST /api/v1/scripts/[scriptId]/analyze`
...
### 5.6 `POST /api/v1/scripts/[scriptId]/comments/analyze`
...
### 5.7 `PUT /api/v1/scripts/[scriptId]/comments/analyze`
...
- 反映先の既定値は `spreadsheetId = 1xEX6oQW_utPzfFDN6sFsk5aT2JwXIcaIQ8mD00fP09Q`、`sheetName = 動画分析`
